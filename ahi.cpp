...
LPVOID AHI::hook_func(LPVOID func_addr, LPVOID dst_func){
    if(func_backups.find(func_addr) != func_backups.end()){
        std::cerr << __FUNCTION__ << ": " << func_addr << " is already hooked!" << std::endl;
        return 0;
    }
    ...
    if(!ReadProcessMemory(process_handle, func_addr, func_backups[func_addr], JMP_OPCODE_SIZE, 0)){
        std::cerr << __FUNCTION__ << ": ReadProcessMemory error: " << GetLastError() << std::endl;
        return 0;
    }
    LPVOID dst_func_addr = (LPVOID)((uintptr_t)dst_func - (uintptr_t)func_addr - (JMP_OPCODE_SIZE - 1));
    DWORD previous_protection;
    VirtualProtect(func_addr, JMP_OPCODE_SIZE, PAGE_EXECUTE_READWRITE,
                   &previous_protection);
    memcpy(&jmp_opcode[1], &dst_func_addr, ADDR_SIZE);
    if(!WriteProcessMemory(process_handle, (LPVOID)func_addr, jmp_opcode, JMP_OPCODE_SIZE,
                       0)){
...
LPVOID AHI::unhook_func(LPVOID func_addr){
    if(func_backups.find(func_addr) == func_backups.end()){
        std::cerr << __FUNCTION__ << ": " << func_addr << " is not hooked!" << std::endl;
        return 0;
    }
    HANDLE process_handle = GetCurrentProcess();
    if(!process_handle){
    ...
    if (!WriteProcessMemory(process_handle, (LPVOID)func_addr, func_backups[func_addr], JMP_OPCODE_SIZE, 0)){
        std::cerr << __FUNCTION__ << ": WriteProcessMemory error: " << GetLastError() << std::endl;
        return 0;
    }
    if(!FlushInstructionCache(process_handle, 0, 0)){
...
LPVOID AHI::hook_dll_func(std::string dll, std::string func_name, LPVOID dst_func_addr){
